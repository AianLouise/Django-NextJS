...
def test_invitation_acceptance():
    ...
    if response.status_code == 201:
        print("✅ Invitation sent successfully")        # Get the invitation token from database
        try:
...
def cleanup_test_data():
    """Clean up test data"""
    try:
        # Delete test organization and its users
        org = Organization.objects.filter(name='Test Accept Org').first()
        if org:
            org.delete()
