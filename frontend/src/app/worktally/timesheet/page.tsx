...
import { LuPlus, LuPencil, LuTrash, LuChevronLeft, LuChevronRight } from 'react-icons/lu';
import { toast } from 'react-hot-toast';
import { apiRequest, TimeEntry } from '@/lib/api';
    ...
    return (
      <div className="bg-white/80 dark:bg-gray-900/70 backdrop-blur-md rounded-2xl lg:rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/30 p-4 sm:p-6 lg:p-8">
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-gray-600 dark:text-gray-400">Loading timesheet...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white/80 dark:bg-gray-900/70 backdrop-blur-md rounded-2xl sm:rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/30 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 sm:mb-8 gap-4">
        ...
        <div className="flex items-center space-x-2 sm:space-x-4 bg-white/50 dark:bg-gray-700/50 rounded-xl p-2 sm:p-3 w-full lg:w-auto">          <button
            ...
            {entriesByDay.map((day) => (
              day.entries.length > 0 ? (day.entries.map((entry: TimeEntry, entryIndex: number) => (
                <tr key={entry.id || entryIndex} className="hover:bg-white/50 dark:hover:bg-gray-700/30 transition-colors duration-200">
                  ...
                  <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-right text-sm font-medium">                    <div className="flex justify-end space-x-1 sm:space-x-2">
                      <button className="p-1.5 sm:p-2 rounded-lg text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/50 transition-all duration-200 min-h-[44px] min-w-[44px] flex items-center justify-center">
                        <LuPencil className="h-3 w-3 sm:h-4 sm:w-4" />
                      </button>
                      <button className="p-1.5 sm:p-2 rounded-lg text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/50 transition-all duration-200 min-h-[44px] min-w-[44px] flex items-center justify-center">
                        <LuTrash className="h-3 w-3 sm:h-4 sm:w-4" />
                      </button>
                    </div>
                  </td>
                </tr>
      ...
      {/* Add Time Entry Button */}      <div className="mt-6 sm:mt-8 flex justify-center sm:justify-end">
